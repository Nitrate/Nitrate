...
class CustomSearchForm(forms.Form):
    ...
    product = ModelChoiceField(
        ...
        error_messages={
            ...
            "invalid_pk_value": "%(pk)s is not a valid product ID for generating this report.",
        },
    )
    ...
    def populate(self, product_id):
        ...
        if product_id:
            f_product_version.queryset = Version.objects.filter(product__id=product_id).only(
                "value"
            )
            self.fields["pk__in"].queryset = TestBuild.objects.filter(product__id=product_id).only(
                "name"
            )
            f_case__category.queryset = TestCaseCategory.objects.filter(
                product__id=product_id
            ).only("name")
            f_case__component.queryset = Component.objects.filter(product__id=product_id).only(
                "name"
            )
        else:
...
class CustomSearchDetailsForm(CustomSearchForm):
    pk__in = ModelChoiceField(
        ...
        error_messages={
            ...
            "invalid_pk_value": "%(pk)s is not a valid test build ID for generating this report.",
        },
    )
...
class BasicTestingReportFormFields(forms.Form):
    ...
    r_product = ModelChoiceField(
        ...
        queryset=Product.objects.only("name").order_by("name"),
        error_messages={
            "required": "You have to select a product to generate this testing report.",
    ...
    r_created_since = forms.DateField(
        ...
        input_formats=["%Y-%m-%d"],
        error_messages={
            "invalid": "The start execute date is invalid. The valid format is YYYY-MM-DD.",
        },
    ...
    r_created_before = forms.DateField(
        ...
        input_formats=["%Y-%m-%d"],
        error_messages={
            "invalid": "The end execute date is invalid. The valid format is YYYY-MM-DD.",
        },
        widget=forms.TextInput(
            attrs={
                "id": "r_created_before",
    ...
    def populate(self, product_id):
        if product_id:
            self.fields["r_build"].queryset = TestBuild.objects.filter(product=product_id).only(
                "name"
            )
            self.fields["r_version"].queryset = Version.objects.filter(product=product_id).only(
                "value"
            )
        else:
