...
class ReportSQL(object):
    # Index
    index_product_plans_count = (
        'SELECT COUNT(plan_id) FROM test_plans WHERE test_plans.product_id = products.id'
    )

    index_product_runs_count = (
        'SELECT COUNT(run_id) '
        'FROM test_runs INNER JOIN test_plans ON test_runs.plan_id = test_plans.plan_id '
        'WHERE test_plans.product_id = products.id'
    )

    index_product_cases_count = (
    ...
    # Custom Search Zone
    # added by chaobin
    case_runs_count_by_status_under_run = (
        'SELECT COUNT(DISTINCT case_run_id) '
        'FROM test_case_runs '
        'WHERE test_case_runs.run_id = test_runs.run_id AND '
        'test_case_runs.case_run_status_id = %s'
    )

    custom_details_case_run_count = (
        'SELECT tcrs.name AS test_case_status, COUNT(tcr.case_id) AS case_run_count '
